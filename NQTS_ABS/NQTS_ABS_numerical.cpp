...
const int dimension = 2; //解維度
const int Cycle = 1000; //迭代次數
const int population_size = 20;//族群大小
const int experiment_times = 1;//實驗次數
...
void IEEE754(){
    ...
    for(int i = 0; i < population_size; i++){
        for(int w = 0; w < dimension; w++){
            ...
            //fraction
            fraction = 0;
            for(int j = 0; j < float_bit-4; j++){
                fraction += pow(0.5, j+1)*Population[i][4+j+w*float_bit];
            }
...
int main(){
    ofstream file;
    string file_name = "./EpinFile/NQTS_ABS_Numerical/NQTS_ABS_Experiment";
    float avg_best = 0;
    float avg_G = 0;
    srand(seed);
    for(int times = 0; times < experiment_times; times++){
        file.open(file_name+to_string(times)+".epin");
        file << "Dimension : 2" << endl;
        file << "Formula : sum(|Xi|)" << endl;
        file << "BP : 0,0" << endl;
        file << "Range : -127 ~ 127" << endl;
        file << "Position : " << endl;
        init();
        for(int G = 0; G < Cycle; G++){
            ...
            for(int i = 0; i < population_size; i++){
                ...
                if(Population_dec_fitness[i][dimension] == Gbest_dec[dimension]){
                    file << "9,0,2";
                }
                else
                    file << "8,0,2";
